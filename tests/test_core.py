...
def test_project_root_resolves_to_right_directory():
    assert str(project_root()) == os.path.abspath(
...
def test_traverse_search_for_file_name(fs):
    files = [Path("/org/dept/project/module/file.txt"), Path("/org/file.txt")]
    for f in files:
        fs.create_file(f)
...
def test_traverse_load_commands_dynamically(fs_patch_open):
    ...
    fs_patch_open.create_file(
        "/org/dept/project/module/.devcli/module.py", contents=DYN_COMMAND
    )
    fs_patch_open.create_file(
        "/org/dept/project/.devcli/project.py", contents=DYN_COMMAND
    )
